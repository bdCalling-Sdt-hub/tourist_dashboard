...
const CategoryTable = ({
  set_selected_data,
  set_open_category_banner_modal,
  setAction,
}) => {
  // states
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "key ",
      render: (_, record) => (
        <div className="start-center gap-3 w-fit">
          {record?.name?.toLowerCase()?.includes("promotion") ? (
            <span className="text-red-500">
              you can't delete or edit promotion category
            </span>
          ) : (
            <>
              <button
